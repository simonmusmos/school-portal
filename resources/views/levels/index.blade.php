	...
	<div class="projects-section-line">
		<div class="projects-status">
			...
			<div class="item-status">
				...
				<span class="status-type">Total Levels</span>
			</div>
		</div>
		<div class="view-actions">
			...
			<button class="view-btn grid-view active" title="Add Level" id="add-level">
			...
			<button class="disabled bulk-delete-btn grid-view active" title="Delete Levels" id="bulk-delete-btn" disabled>
...
@include('levels.partials.modal') 
@endsection
@push('scripts')
	<script>
		var storeURL = "{{ route('levels.store') }}";
		var getDataURL = "{{ route('levels.get') }}";
		var deleteURL = "{{ route('levels.destroy') }}";
		var getCountDataURL = "{{ route('levels.getCount') }}";
		...
		$("#add-level").on("click", function () {
			...
			$("#formModal").find(".modal-title").html("Add Level");
			$("#formModal").find("#active_div").addClass("d-none");
			$("#formModal").find("button[type='submit']").html("Add Level");
		});
			...
			$.ajax({
				...
				success: function(response) {
					if (type == 'formModal') {
						...
						$("form input[name='name']").val(response.level.name);
						$("form input[name='id']").val(response.level.id);
						$("form input[name='active']").prop("checked", false);
						if (response.level.is_active == "1") {
							$("form input[name='active']").prop("checked", true);
						}
						showModal('formModal');
						$("#formModal").find("#active_div").removeClass("d-none");
						$("#formModal").find(".modal-title").html("Edit Level");
			...
			if (type == 'table') {
				...
				$.each(data.levels, function (index, value) {
					...
					rows += '<tr>'+
							...
							'	<th class="object-name" scope="row" style="color: #666666;">' + value.name + '</th>'+
							'	<td>0</td>'+
							'	<td>0</td>'+
							'	<td class="text-status-' + status.toLowerCase() + '">' + status + '</td>'+
							'	<td>'+
							'		<div class="btn-group">'+
							'			<button type="button" class="edit-btn" data-value="' + value.id + '" title="Edit Level">'+
							...
							'			</button>'+
							'			<button type="button" class="delete-btn" data-value="' + value.id + '" title="Delete Level">'+
							'				<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>'+
							'			</button>'+
							'		</div>'+
							'	</td>'+
